...
### 11.2.1 위임 가능한 플랜 생성 기준
- 별도 세션에 구현을 위임할 플랜은 "아이디어 메모"가 아니라 "실행 계약"이어야 한다.
- 위임 가능한 플랜에는 아래 항목을 포함하는 것을 원칙으로 한다.
  - 목표/비목표
...
- 플랜에 다중 해석 여지가 남아 있거나, 구현 세션이 설계를 새로 결정해야만 진행 가능한 상태라면 위임 가능한 플랜으로 간주하지 않는다.
- 구현 세션이 문서를 수정해야 하는 경우, 어떤 문서를 어느 범위까지 갱신할지 플랜에 명시한다.

### 11.2.2 플랜 첨부 구현 요청 처리 규칙
- 사용자가 "플랜 첨부 + 구현 요청" 형태로 요청하면, 첨부 플랜을 현재 단계의 SSOT로 간주하고 구현한다.
- 구현 세션은 로컬 코드베이스 탐색을 통해 파일 위치, 기존 네이밍, 배치 방식 같은 로컬 세부사항만 보완한다.
