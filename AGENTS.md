...
## 8. 문서/결정 기록(ADR)
- 아래 조건 중 하나 이상에 해당하는 **중요 결정**만 `Docs/ADR/ADR-YYYYMMDD-NN-*.md`에 기록한다(해당하지 않는 설계/변경에 대해서는 의무가 아니다):
...
- 답변에서 프로젝트 내부 파일을 참조할 때는 마크다운 링크나 절대 경로를 사용하지 않고, 프로젝트 루트 기준 상대 경로의 plain text 형식으로만 표기한다.
...
## 11. Agent 작업 절차
### 11.1 기본 원칙
- Agent는 기본적으로 사용자 요청을 빠르게 구현한다.
- 설계/계획 승인은 **고영향 변경**에서만 필수로 요구한다.
- 저영향 변경은 절차를 축약하거나 생략할 수 있다.
...
### 11.3 저영향 변경(즉시 진행 가능)
- 아래는 승인 없이 바로 진행 가능하다.
  - 단순 버그 수정
  - 명확한 범위의 리팩터링
  - 문서/주석/네이밍 정리
  - 기존 규칙 안에서의 구현 보완
- 필요 시 작업 중간에 핵심 변경점만 짧게 공유한다.

### 11.4 최소 절차(권장)
1. 필요한 경우에만 간단 계획 공유(생략 가능)
2. 구현
3. 가능한 범위에서 검증(EditMode/PlayMode 스모크 우선)
4. 결과 요약(변경 내용, 검증 결과, 남은 리스크)
