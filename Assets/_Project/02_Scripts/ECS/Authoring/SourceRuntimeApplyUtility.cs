            ...
            for (int s = 0; s < clip.Segments.Length; s++)
            {
                ...
                for (int e = 0; e < entryCount; e++)
                {
                    ...
                    clipPatternBuffer.Add(new SourceClipPatternBuffer
                    {
                        ...
                        LocalEndSec = endSec,
                        ProfileRefId = snapshot.EmissionCore.ProfileRefId,
                        BulletTypeKey = typeKey,
                        HasSpeedOverride = snapshot.EmissionCore.HasSpeedOverride ? (byte)1 : (byte)0,
                        SpeedOverride = snapshot.EmissionCore.SpeedOverride,
                        HasLifetimeOverride = snapshot.EmissionCore.HasLifetimeOverride ? (byte)1 : (byte)0,
                        LifetimeOverride = snapshot.EmissionCore.LifetimeOverride,
                        HasMovementOverride = snapshot.EmissionCore.HasMovementOverride ? (byte)1 : (byte)0,
                        MovementFamily = snapshot.EmissionCore.MovementFamily,
                        DampedLinear = snapshot.EmissionCore.DampedLinear,
                        HomingLite = snapshot.EmissionCore.HomingLite,
