...
## 운영 원칙
1. 문서 포맷 강제는 최소화한다. 필수는 파일명 포맷과 Metadata만 유지한다.
2. 파일명 포맷(필수): 각 분류 문서는 `GD/TD/OPS-###-kebab-case.md`, ADR은 `ADR-YYYYMMDD-NN-kebab-case.md`를 사용한다.
3. Metadata(필수): 문서 상단 `## Metadata` 블록은 유지한다.
4. 본문 섹션 구성은 세션 제안(권장)이다: 템플릿 구조, 섹션 순서, 상세 목차는 상황에 맞게 추가/삭제/재배치할 수 있다.
5. ADR 작성/링크 갱신/상대경로 링크는 운영 효율을 위한 권장 규칙이다.
...
- [Docs/Templates/technical-design-template.md](Templates/technical-design-template.md)
- [Docs/Templates/project-ops-template.md](Templates/project-ops-template.md)
