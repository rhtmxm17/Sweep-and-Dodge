...
### 5.7 Source 강조 상태 체감 규칙
- `Baseline`:
  - 스테이지 기본 압력을 유지하는 상태다.
  - 플레이어가 "지금은 정리 리듬을 유지하는 구간"으로 해석할 수 있어야 한다.
- `Pressure`:
  - 특정 Source가 전면으로 올라오는 상태다.
  - 플레이어가 "지금은 이 Source를 우선 처리해야 한다"는 압박을 체감해야 한다.
- `Finish`:
  - Source 정리 완료 이후의 마무리 상태다.
  - 플레이어가 "이 구역은 끝났고 다음 구역으로 넘어간다"는 전환 감각을 가져야 한다.
- 상태 선정 알고리즘/가중치/타이브레이크/클립 전환 계약은 [TD-006](../TechnicalDesign/TD-006-run-progress-director-design.md)에서 관리한다.

## 6. 문서 경계
- 필드 단위 스키마, 계산식, 검증 규칙, 런타임 반영 절차는 [TD-002](../TechnicalDesign/TD-002-pattern-wave-progress-runtime-contract.md)에서 관리한다.
- SpawnDirective 분해 모델(Sampling/Emission/Payload)과 조합 규칙 상세는 [TD-003](../TechnicalDesign/TD-003-spawn-directive-model.md)에서 관리한다.
- Source 강조 상태(`Baseline/Pressure/Finish`)의 전환 계약과 선정 규칙은 [TD-006](../TechnicalDesign/TD-006-run-progress-director-design.md)에서 관리한다.
