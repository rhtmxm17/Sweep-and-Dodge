...
- 예외: `Finish` 진입 시점은 `SourceState -> Depleted` 전환과 결합된 강제 교체 시점으로 본다.
  ...
  - `Finish` 전환 시점의 1회성 연출은 별도 연출 계약으로 분리한다.

## 9. 변경 이력
- 2026-03-05: 후속 논의 섹션을 구현 반영 기준으로 정리하고, 미확정 항목을 `Finish` 1회성 연출 범위로 축소했다.
...
- 2026-02-26: `Finish` 지속 Clip은 `Trash Lane`만 허용하고, 전환 시점 1회성 연출은 후속 결정 항목으로 정리했다.
