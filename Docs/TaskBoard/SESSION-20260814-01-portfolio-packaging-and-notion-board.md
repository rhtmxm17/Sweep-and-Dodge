...
- 자동 테스트, PlayMode smoke, 성능 측정은 현재 단계에서 독립적인 핵심 역량 주장보다 프로젝트 결과를 뒷받침하는 보조 증거로 취급한다.
- 테스트 도입은 프로젝트 안정화를 위해 agent가 제안한 방식을 수용하면서 시작됐다. 본인이 테스트 적용 맥락과 전략을 어느 수준까지 이해하고 설명할 수 있는지는 T2에서 대표 사례별로 확인한 뒤 공개 강조 수준을 결정한다.
- 이해와 근거가 확인되기 전에는 포괄적인 테스트 전략 수립 능력이나 성능 엔지니어링 전문성을 주장하지 않는다.
...
## Now
- [ ] T2. 주장-근거 인벤토리를 작성한다.
  - 완료 기준: 각 핵심 주장에 코드/설계 문서/테스트/수치/영상/빌드 근거와 부족한 증거가 연결된다.
  - 검증: 근거가 없는 주장을 성과 표현으로 확정하지 않고, development snapshot과 public benchmark가 구분된다.
  - 검증: 대표 테스트와 측정 자료마다 도입 맥락, 보호하는 계약, 실패 시 의미, 한계, 본인의 설명 가능 수준을 확인해 `핵심 주장`, `보조 증거`, `비공개` 중 적절한 노출 수준을 정한다.
  - 근거: `PORT-001`, `PORT-003`, `SESSION-20260514-01`.

## Next
- [ ] T3. 공개 데모와 검증 기준을 확정한다.
  - 완료 기준: 대표 플레이 구간, 성능 시나리오, 측정 환경·지표, 촬영 목록, 빌드 전달 기준이 정해진다.
  - 검증: 결과를 재현하거나 최소한 측정 조건과 해석 범위를 확인할 수 있다.
  - 근거: `PORT-003`, `SESSION-20260514-01`.
...
## End of Session
- 결과: T1 포트폴리오 브리프를 완료하고 모든 채택 기준을 `Adopted Baseline`에 고정했다.
- 남은 리스크: T2에서 각 주장과 검증 자료의 실제 근거, 설명 가능 수준, 공개 노출 강도를 아직 확인해야 한다.
- 다음 시작점: T2 주장-근거 인벤토리를 별도 대화 또는 분기에서 시작한다.
