...
- 총 개발 기간: 2개월
- 현재 기준일: 2026-03-04
- 목표 결과물: 외부 시연 가능한 단일 데모 빌드(코어 루프 + 최소 UI/사운드/결과 흐름)
- 아트 자산 제작은 범위에서 제외한다.
- 단, Animator/파티클/사운드가 연결될 수 있는 시스템 구조와 이벤트 계약은 범위에 포함한다.

## 3. 데모 완료 정의 (Demo DoD)
- 플레이어가 `Start -> Stage Run -> Clear/Fail -> Result -> Retry/Exit`를 끊김 없이 수행할 수 있다.
...
|---|---|---|---|---:|---|
| S1 | StageFlow/UI 연동 | 임시 호출자 제거, 실제 Start/Confirm/Clear 플로우 확정 | WIP | 2.0 | P0 |
| S2 | 플레이 HUD | 디버그 HUD와 별개로 플레이어 HUD 최소 세트 확정 | TODO | 1.5 | P0 |
| S3 | 결과/실패 UX | Clear/Fail 판정 및 Result 화면 지표/재시작 동선 확정 | TODO | 1.5 | P0 |
...
## 5. 우선순위 백로그 (세부)
1. `P0` StageFlow/UI 실연동 계약
- `RunDirectorStageTempFlowDriver` 대체 계획 수립
- Stage 시작 기본 모드(`InitialStageState`) 운영 기준 확정
- Clear/Completed 신호를 화면 전환과 연결
...
## 7. 설계 문서 분해 계획 (권장)
- TD 후속(권장):
  - `TD-010`: Demo StageFlow/UI Presentation Contract
  - `TD-011`: Runtime Player HUD Contract
  - `TD-012`: Audio Event Routing And Mixer Policy
- GD 후속(권장):
  - `GD-008`: Demo Onboarding And Result UX
- ADR 기록 기준:
