...
| 9 | 공통 전투 이벤트 채널 (피격/수집/정리 집계) | P1 | 4 | 8% | WIP | 20% | 공통 채널 Producer/Consumer 구현 + 기존 Hit 경로 이관 | 2026-02-28 |
...
9. 공통 전투 이벤트 채널: 피격/수집/정리 이벤트를 단일 집계 경로로 통합해 연출, 점수, 통계가 같은 소스를 보게 한다.
...
- `#8` 명칭을 `런 타임라인 디렉터`에서 `런 진행도 디렉터`로 갱신했다.
  ...
  - 스테이지(2~3분), 캠페인(무실패 15~20분)은 페이싱 가드레일로 유지한다.
- 문서 내 레거시 Build 시스템 명칭을 실제 코드명 `SourceClipRequestBuildSystem`으로 통일했다.
...
- `#7 데이터 주도 패턴 정의` 진행률을 상향했다.
  - Spawn 규약 업데이트를 반영했다:
    ...
    - 근거/참조 문서: `TD-003`(SpawnDirective 모델), `TD-005`(WaveClip 인라인 설정 레퍼런스)
  - 후속 작업:
...
- `#2 Spawn/Despawn 요청-소비 구조`를 `DONE`으로 갱신했다.
  - `SourceClipRequestBuildSystem` + `SpawnRequestRoundRobinExecutionSystem` + `SpawnBacklogWarningSystem` 분리/정리 반영.
