...
## 3. 진행 현황 스냅샷
- 전체(10): `DONE 5 | WIP 2 | TODO 3 | BLOCK 0` (완료율 50%)
- `P0`(5): 5/5 완료 (100%)
- `P1`(4): 0/4 완료 (0%) + `WIP 2`
- `P2`(1): 0/1 완료 (0%)
...
| 7 | 데이터 주도 패턴 정의 (패턴/웨이브/보상 분리) | P1 | 4 | 14% | WIP | 25% | `GD-007`/`TD-002` 기준으로 MVP 필드/검증 규칙 확정 + `ContentValidationRules.cs` 1차 반영 | 2026-02-23 |
...
7. 데이터 주도 패턴 정의: 방향성은 `GD-007`, 런타임 계약은 `TD-002`로 분리해 패턴/웨이브/진행도의 기획-구현 경계를 고정하고 반복 실험 속도를 높인다.
